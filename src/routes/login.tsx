...
export const Route = createFileRoute("/login")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Accede a tu cuenta de Aprendo English como Alumno, Familia o Profesor.",
      },
...
function LoginPage() {
  ...
  // Código de alta. Sólo hace falta para quien se registra con un correo
  // personal: con un correo institucional, el dominio ya lo asigna solo (ver
  // assign_org_on_signup en la migración de instituciones).
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === "signup") {
        ...
        if (data.session) {
          // El código se canjea DESPUÉS de tener sesión: redeem_org_invite se
          // apoya en auth.uid(). Si falla no se interrumpe el alta —la cuenta
          // ya existe—; sólo se queda sin institución y se le puede asignar
          // desde /instituciones.
          if (invite.trim()) {
            const r = await redeemInvite({ data: { code: invite } }).catch(() => null);
            if (!r?.ok) setErr("La cuenta se creó, pero el código de institución no es válido.");
          }
        ...
        <form onSubmit={submit} className="space-y-3">
          ...
          {mode === "signup" && (
            <div>
              <input
                type="text"
                placeholder="Código de tu institución (opcional)"
                value={invite}
                onChange={(e) => setInvite(e.target.value.toUpperCase())}
                className="w-full px-4 py-2 border rounded-lg font-mono tracking-wide"
                maxLength={24}
              />
              <p className="text-[11px] text-slate-400 mt-1">
                Si te registras con el correo de tu institución no hace falta.
              </p>
            </div>
          )}
          <input
